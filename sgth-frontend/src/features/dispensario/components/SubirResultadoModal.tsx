...
export function SubirResultadoModal({
  opened, onClose, consulta, historiaClinicaId,
}: Props) {
  ...
  const handleSubmit = () => {
    if (!tipo) { setError('Seleccione el tipo de resultado.'); return }
    if (!descripcion.trim()) { setError('Ingrese una descripción.'); return }
    if (!archivo) { setError('Seleccione un archivo.'); return }
    if (!fecha) { setError('Ingrese la fecha del resultado.'); return }
    ...
    >
      <Stack gap="sm">
        <Select
          ...
          {...contained}
          value={tipo}
          onChange={(v) => { setTipo(v ?? ''); setError(null) }}
        />
        ...
        <FileInput
          ...
          {...contained}
          value={archivo}
          onChange={(f) => { setArchivo(f); setError(null) }}
        />
