...
export function AsignarServidorModal({ opened, onClose, usuario }: Props) {
  ...
  const asignar = useMutation({
    mutationFn: (servidorId: number) =>
      usuarioService.asignarServidor(Number(usuario?.id), servidorId),
    onSuccess: () => {
      notifications.show({
    ...
    >
      <Stack gap="md">
        <Text size="sm" c="dimmed">
          Busca el servidor que deseas vincular a este usuario.
        </Text>
        ...
        {resultados.length > 0 && (
          <Stack gap="xs">
            <Text size="xs" c="dimmed">
              {resultados.length} resultado(s) — selecciona el servidor a
              vincular:
            </Text>
            {resultados.map((s) => (
