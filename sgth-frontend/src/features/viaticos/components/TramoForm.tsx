...
import {
  Stack, Grid, Select, Button,
  Divider, Alert, Text, Group,
} from '@mantine/core'
...
import type { CatalogoTransporte, EmpresaTransporte,
              Viatico } from '@/types/api'
import { tramoSchema, type TramoFormData } from '../schemas/viatico.schema'

const PAISES_COMUNES = [
  'Colombia','Perú','Chile','Estados Unidos','España',
  'Brasil','México','Panamá','Argentina','Bolivia',
  'Costa Rica','Ecuador','Uruguay','Paraguay','Venezuela',
  'Francia','Suiza','Alemania','Italia','Bélgica',
  'Reino Unido','Canadá','China','Japón','Corea del Sur',
  'República Dominicana','Guatemala','El Salvador',
  'Honduras','Nicaragua','Cuba','Países Bajos',
  'Austria','Israel','India','Australia','Otro',
...
export function TramoForm({
  viaticoId, viatico, tramosExistentes,
  onSuccess, onCancel,
}: Props) {
  ...
  const empresaOptions = (empresas as EmpresaTransporte[]).map(e => ({
    value: String(e.id), label: e.nombre ?? '',
  }))

  const alertaSalida: 'ok' | 'error' | null = viatico && salidaTramo
    ? (() => {
  ...
  const alertaLlegada: 'ok' | 'error' | null = viatico && llegadaTramo
    ? (() => {
  ...
  const crear = useMutation({
    mutationFn: (
      data: Parameters<typeof viaticoService.tramos.crear>[1]
