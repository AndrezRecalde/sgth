...
import {
  ...
  Button, Group, Divider, Alert, Text,
  Card, ThemeIcon, MultiSelect,
} from '@mantine/core'
...
import {
  ZONA_OPTIONS, MODALIDAD_OPTIONS,
  TIPO_VIAJE_OPTIONS, PAISES_OPTIONS,
...
export function ViaticoModal({ opened, onClose, onCreated }: Props) {
  ...
  const { data: miPerfil } = useQuery({
    queryKey: ['mi-perfil'],
    queryFn:  () =>
      api.get<{ datos: MiPerfil }>('/auth/me')
        .then(r => r.data.datos),
    enabled:   opened,
    staleTime: 1000 * 60 * 5,
  })
    ...
    <Modal
      ...
      title={
        <Group gap="xs">
          ...
          <Text fw={600} size="md">Nueva solicitud de viático</Text>
        </Group>
      }
      size="xl"
      fullScreen={isMobile}
        ...
        <Stack gap="md">
          ...
          <Grid>
            ...
            <Grid.Col span={{ base: 12, sm: 2 }}>
              <div>
                <Text size="xs" c="dimmed" mb={4}>Total días</Text>
          ...
          {zonaWatch === 'exterior' && (
            <>
              ...
              >
                ...
                <Text size="xs" mt={2}>
                  El monto se calculará cuando el gestor apruebe
                  la solicitud, aplicando la tarifa y coeficiente
                  del país de destino.
                </Text>
              </Alert>
              <Grid>
              ...
              <Alert color="blue" variant="light" p="xs"
                ...
                <Text size="xs">
                  El gestor calculará el monto al aprobar
