...
export function FacturasResumen({ viatico, totalFacturas }: Props) {
  ...
  const diferencia = modalidad === 'sin_anticipo'
    ? 0
    : monto70 - totalFacturas

  const porcentajeJustif = monto70 > 0
  ...
  return (
    <Card withBorder radius="md" p="sm" bg="gray.0">
      <Group justify="space-between">
        <Text size="sm" c="dimmed">Monto total asignado:</Text>
        <Text size="sm" fw={600}>${montoAsignado.toFixed(2)}</Text>
      </Group>
      <Group justify="space-between">
        <Text size="sm" c="dimmed">70% a justificar (H&A):</Text>
        <Text size="sm" fw={600}>${monto70.toFixed(2)}</Text>
      </Group>
      {anticipo > 0 && (
        <Group justify="space-between">
          <Text size="sm" c="dimmed">Anticipo entregado:</Text>
          <Text size="sm" fw={600}>${anticipo.toFixed(2)}</Text>
        </Group>
      )}
      <Group justify="space-between">
        <Text size="sm" c="dimmed">Total comprobantes:</Text>
        <Text size="sm" fw={600}
      ...
      <Divider my={4} />
      <Group justify="space-between">
        <Text size="sm" fw={600}>A devolver a la institución:</Text>
        <Text size="sm" fw={700}
          c={justificadoCompleto ? 'teal' : 'orange'}>
          {diferencia >= 0
      ...
      {!justificadoCompleto && diferencia > 0 && (
        <Alert color="yellow" variant="light" p="xs" mt={4}>
          <Text size="xs">
            Faltan <strong>${diferencia.toFixed(2)}</strong>
