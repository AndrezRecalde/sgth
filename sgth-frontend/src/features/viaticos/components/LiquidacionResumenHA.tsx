...
export function LiquidacionResumenHA({
  montoAsignado, montoAnticipo, monto70, monto30,
  totalHospAli, totalMovilizacion, porcentajeHA,
  justificadoCompleto, diferenciaDevolver, modalidad,
}: Props) {
    ...
    <Stack gap="sm">
      <Card withBorder radius="md" p="sm">
        ...
        <Group justify="space-between">
          ...
          <Text size="xs" fw={600}>${montoAsignado.toFixed(2)}</Text>
        </Group>
        <Group justify="space-between">
          <Text size="xs" c="dimmed">70% a justificar (H&A):</Text>
          <Text size="xs" fw={600}>${monto70.toFixed(2)}</Text>
        </Group>
        {montoAnticipo > 0 && (
          <Group justify="space-between">
            <Text size="xs" c="dimmed">Anticipo entregado:</Text>
            <Text size="xs" fw={600}>${montoAnticipo.toFixed(2)}</Text>
          </Group>
        )}
        <Divider my={4} />
        <Group justify="space-between">
