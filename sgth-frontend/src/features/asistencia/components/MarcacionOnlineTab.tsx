...
import { Stack, Button, Text, Badge, Group,
         Card, Alert, Loader, Center } from '@mantine/core'
import {
  IconLogin, IconLogout, IconSoup, IconInfoCircle, IconFingerprint,
  ...
  return (
    <Stack gap="md" maw={500} mx="auto">
      <Alert icon={<IconInfoCircle size={16} />} color="blue" variant="light">
        <Text size="xs">
          Esta función es para servidores en campo o territorio.
          Registra tu asistencia desde cualquier lugar con conexión a internet.
        </Text>
      </Alert>

      {/* Estado del día */}
      <Card withBorder radius="md" p="md">
        <Text size="sm" fw={600} mb="xs">Estado de hoy</Text>
        {cargandoEstado ? (
          <Center><Loader size="sm" /></Center>
        ) : estadoHoy ? (
          <Stack gap="xs">
            {(['Entrada', 'AlmuerzoSalida', 'AlmuerzoRetorno', 'Salida'] as const).map(campo => (
              <Group key={campo}>
                <Text size="xs" c="dimmed" w={130}>
                  {campo === 'Entrada' ? 'Entrada'
                    : campo === 'AlmuerzoSalida' ? 'Salida almuerzo'
                    : campo === 'AlmuerzoRetorno' ? 'Retorno almuerzo'
                    : 'Salida'}:
                </Text>
                <Badge
          ...
          <Text size="sm" c="dimmed">Sin marcaciones hoy.</Text>
        )}
      </Card>
      ...
      {cargandoUbicacion ? (
        <Group gap="xs">
          <Loader size="xs" />
          <Text size="xs" c="dimmed">Obteniendo ubicación...</Text>
        </Group>
      ) : ubicacion ? (
        ...
        <Alert color="orange" variant="light">
          <Text size="xs">
            No se pudo obtener la ubicación. Activa el GPS para registrar tu posición.
          </Text>
        </Alert>
      )}
      ...
      <Stack gap="sm">
        <Button
          size="lg" color="emerald" variant="filled"
        ...
        <Button
          size="lg" color="blue" variant="light"
          leftSection={<IconSoup size={20} />}
          loading={registrando}
        ...
        <Button
          size="lg" color="red" variant="light"
