...
import type { DiscapacidadServidor, EnfermedadCatastroficaServidor, ApiResponse } from '@/types/api'
import type { DataTableColumn } from 'mantine-datatable'

interface Props {
  servidorId:                    number
  tieneDiscapacidad?:            boolean
  tieneEnfermedadCatastrofica?:  boolean
}

export function CondicionTab({
  servidorId,
  tieneDiscapacidad,
        ...
        <TableActions actions={[
          {
            ...
            color: 'red',
            onClick: () => {
              if (confirm('¿Eliminar este registro de enfermedad catastrófica?'))
    ...
    <Stack gap="lg">
      <div>
        <Group justify="space-between" mb="xs">
          <Divider
            label={<Text size="sm" fw={600}>Discapacidades</Text>}
            labelPosition="left"
            style={{ flex: 1 }}
          />
          <Button size="xs" color="emerald" variant="light"
            leftSection={<IconPlus size={14} />} onClick={openDisc}>
            Registrar
          </Button>
        </Group>
      ...
      <div>
        <Group justify="space-between" mb="xs">
          <Divider
            label={<Text size="sm" fw={600}>Enfermedades catastróficas</Text>}
            labelPosition="left"
            style={{ flex: 1 }}
          />
          <Button size="xs" color="emerald" variant="light"
            leftSection={<IconPlus size={14} />} onClick={openEnf}>
            Registrar
          </Button>
        </Group>
      ...
      <DiscapacidadModal
        opened={discOpened} onClose={closeDisc} servidorId={servidorId} />
      <EnfermedadModal
        opened={enfOpened} onClose={closeEnf} servidorId={servidorId} />
    </Stack>
  )
}
