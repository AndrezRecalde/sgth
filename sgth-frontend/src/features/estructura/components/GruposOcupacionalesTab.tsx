  ...
  const columns: DataTableColumn<GrupoOcupacional>[] = [
    ...
    },
    {
      ...
      render: ({ grupo, denominacion_generica }) => (
        <div>
          <Text size="sm" fw={500}>{grupo}</Text>
          {denominacion_generica && (
            <Text size="xs" c="dimmed">{denominacion_generica}</Text>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      title: 'Complejidad',
      width: 110,
      render: ({ nivel_complejidad }) => nivel_complejidad ? (
        <Badge
          color={COMPLEJIDAD_COLORS[nivel_complejidad] ?? 'gray'}
          variant="dot"
          size="sm"
        >
          {nivel_complejidad.charAt(0).toUpperCase() + nivel_complejidad.slice(1)}
        </Badge>
